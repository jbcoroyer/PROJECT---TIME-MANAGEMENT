...
vi.mock("../server/billingOrg", () => ({
  finalizeExpiredTrial: vi.fn(async (_orgId, plan, trialEndsAt) => {
...
describe("resolveBillingAccess", () => {
  ...
  it("bloque après expiration de l'essai", async () => {
    const supabase = mockSupabase({
      profile: { organization_id: "org-1" },
    ...
    expect(result.plan).toBe("canceled");
  });

  it("autorise un abonnement actif", async () => {
    const supabase = mockSupabase({
      profile: { organization_id: "org-1" },
      org: { plan: "active", billing_status: "active", trial_ends_at: null },
    });
