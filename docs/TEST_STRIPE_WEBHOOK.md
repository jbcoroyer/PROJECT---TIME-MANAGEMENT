...
| `STRIPE_PRICE_STARTER` | ID du prix Starter en mode test (`price_…`) |
| `STRIPE_PRICE_PRO` | ID du prix Pro en mode test (`price_…`) |
...
- `plan` déduit du `price_id` (`STRIPE_PRICE_STARTER` / `STRIPE_PRICE_PRO`) ou de `metadata.plan`
...
3. Aller sur `/billing` et lancer un checkout **Starter** ou **Pro** (carte test Stripe : `4242 4242 4242 4242`).
...
Attendu : `stripe_customer_id` et `stripe_subscription_id` renseignés, `plan` = `starter` ou `pro`, `billing_status` = `active` ou `trialing`.

5. Dans le **Dashboard Stripe (mode test)** → **Clients** → ouvrir le client → modifier l’abonnement (changer de plan, annuler, simuler impayé) pour déclencher `customer.subscription.updated`, `customer.subscription.deleted` ou `invoice.payment_failed`.
...
- [ ] `STRIPE_PRICE_STARTER` et `STRIPE_PRICE_PRO` correspondent aux produits **live** actifs.
