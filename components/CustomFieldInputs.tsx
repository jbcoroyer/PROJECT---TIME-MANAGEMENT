...
function renderFieldInput(
  ...
  switch (field.type) {
    ...
    case "checkbox":
      if (readOnly) {
        return <ReadonlyValue>{value === true ? "Oui" : "Non"}</ReadonlyValue>;
      }
      return (
    ...
    case "select":
    case "status":
      return <SelectField field={field} value={value} readOnly={readOnly} onChange={onChange} />;
    case "person":
      return (
        <EntitySelect
...
export default function CustomFieldInputs({ fields, values, onChange, readOnly }: Props) {
  const sorted = useMemo(
    () => [...fields].sort((a, b) => a.position - b.position || a.label.localeCompare(b.label, "fr")),
    [fields],
  );
